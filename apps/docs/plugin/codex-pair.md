...
| `/codex-pair-ack <hash> "<reason>"` | Acknowledge a HIGH finding without fixing it (known trade-off or tracked in a ticket). The `<hash>` is the content hash shown alongside the finding when the Stop gate blocks. The reason is recorded to `.codex-pair/state/acks.json` with a timestamp; acknowledged findings are skipped by the gate until the file changes |
