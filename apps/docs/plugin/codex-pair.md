...
| `codex-pair-session` (SessionStart) | At Claude session start | Announces a paused project and starts a session broker unless disabled |
| `codex-pair-session` (SessionEnd) | At Claude session end | Clears debounce state and tears down the session broker |
...
The broker runs `codex app-server` in the background for each session. It uses a private temporary Codex home containing access to authentication, without the user's hooks, rules, or MCP configuration. SessionEnd removes the process and private home; the next SessionStart replaces a broker left by a session that ended unexpectedly. If startup, transport, health, or protocol handling fails, that edit uses the per-edit `codex exec` path. A session where SessionStart never ran also uses the direct path. The measured cold start was under one second and warm reviews were no faster, so enabling the broker is not a promised latency improvement.

To disable it for one project, put `broker: false` in `.codex-pair/context.md` frontmatter:
...
`ASK_CODEX_BROKER=0` always disables it. Project `broker: false` also wins over `ASK_CODEX_BROKER=1`; otherwise the broker is enabled. These settings affect the broker only, so codex-pair reviews continue through the direct path.
