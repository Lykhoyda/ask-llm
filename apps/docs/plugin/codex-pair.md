...
The broker runs `codex app-server` in a private temporary Codex home without user hooks, rules, or MCP configuration and with the built-in apps connector disabled. It shares your credentials rather than copying them: the home links your `auth.json`, and Codex rewrites that file in place, so a token refresh inside the broker updates your own credentials like any other Codex process. Without an `auth.json` (keyring or environment-key setups) the broker does not start. To disable it for one project, put `broker: false` in `.codex-pair/context.md` frontmatter. `ASK_CODEX_BROKER=0` always disables it, including when the project has no broker setting.
...
Cold start measured under one second and warm reviews were no faster, so default-on is not a latency promise. The broker is TypeScript executed directly by Node, so it runs on Node 22.18+ or 23.6+; on older Node, reviews use `codex exec` without the broker.
