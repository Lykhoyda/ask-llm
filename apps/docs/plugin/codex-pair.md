...
| `codex-pair-session` (SessionStart) | At Claude session start | Announces a paused project and starts the broker when enabled |
...
| `ASK_CODEX_TIMEOUT_MS` | `800000` | Review timeout (inherited from `@ask-llm/codex-mcp`); an enabled broker gets half this budget, leaving the remainder for direct fallback |
| `ASK_CODEX_REASONING_EFFORT` | `medium` | Reasoning effort for `gpt-6-sol` pair reviews. `/codex-review` and `/brainstorm` stay at `high` |
| `ASK_CODEX_BROKER` | disabled | Set to `1` to opt in to the broker unless the project sets `broker: false`; otherwise use per-edit `codex exec` |
...
The broker is opt-in because it links your `auth.json`; a token refresh can rewrite that shared file, and a safer credential strategy has not been established. Set `ASK_CODEX_BROKER=1` to enable it. It runs `codex app-server` in a private temporary Codex home without user hooks, rules, or MCP configuration and with the built-in apps connector disabled. Without an `auth.json` (keyring or environment-key setups) the broker does not start. To disable it for one project, put `broker: false` in `.codex-pair/context.md` frontmatter. `ASK_CODEX_BROKER=0` also disables it.

Startup, transport, health, and protocol failures use the per-edit `codex exec` path. A quota error from the broker goes straight to the fallback model, and a transient provider error (502/503/504, connection resets) gets one direct retry. A broker turn gets half the review timeout, leaving the remaining time for its first direct fallback attempt; transient retries and fallback-model attempts retain their own timeouts. The broker's socket path is inside its private home, so deep project paths do not matter; Codex 0.156.1 makes that path a symlink to a socket in its shared per-user daemon directory, and teardown removes only the symlink. A recorded broker is only signaled or reused while its process is still `codex app-server` on the recorded socket; a dead process, an expired 24-hour owner lease, or a broker bound to a different credential home is replaced on the next opted-in SessionStart. If `ps` cannot verify a recorded broker, SessionStart leaves its descriptor and private home intact and starts no replacement; the next SessionStart or SessionEnd can check again. An opted-in session without its own `auth.json` never stops another session's healthy broker. A SessionStart killed mid-start is recovered by the next one when `ps` can check for its orphan. An opted-out SessionStart leaves another session's healthy broker running and retires it only if the broker's own credential source is absent. A broker left running after a missed SessionEnd can therefore remain until an opted-in SessionStart recovers it, its owner ends, or its credential source disappears; there is no global orphan reaper. Overlapping Claude sessions in one project share one broker: when the session that started it ends, the others review through `codex exec` until a new session starts one.

Cold start measured under one second and warm reviews were no faster. The hooks, the broker, and every helper script are written in TypeScript and ship as generated JavaScript, so they run from a marketplace checkout or an npm install without a build step; they support Node 24 (the current LTS) or newer.
