...
**The executor handles this automatically**: Gemini falls back from `gemini-3.1-pro-preview` to `gemini-3.8-flash`, and Codex falls back from `gpt-5.6-sol` to `gpt-5.6-terra`. You'll see `usage.fellBack: true` in the structured response.
