...
- `@ask-llm/plugin` is the one publishable Claude Code + Pi host package (ADR-142); `docs/PI-COMPATIBILITY.md` is the skill disposition. Pi deliberately excludes `fable-review` and the codex-pair skill family, uses native provider tools (not MCP), and provides no pairing lifecycle in this release. Claude codex-pair remains unchanged.
