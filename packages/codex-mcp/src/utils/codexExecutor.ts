    ...
    try {
      ...
      if (isQuotaError(error) && model !== MODELS.FALLBACK) {
        ...
        try {
          ...
          // The pinned fallback is structurally rejected for this account type (a
          // 400, not a quota) — the ladder is broken in a way `codex doctor` can't
          ...
          if (isModelUnavailableError(fallbackError)) {
            throw new Error(
              `${MODELS.DEFAULT} quota exceeded and the fallback model "${MODELS.FALLBACK}" is not available for this Codex account type (${fallbackMsg}). Set ASK_CODEX_FALLBACK_MODEL to a model your account supports — gpt-5.4-mini (the default) works on both ChatGPT-plan and API-key accounts — or unset it to use the default.`,
            );
          }
          throw new Error(
