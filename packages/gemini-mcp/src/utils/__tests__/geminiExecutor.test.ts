...
describe("executeGeminiCLI quota fallback", () => {
  ...
  // #312 — gemini-3.8-flash (GA 2026-09-02) supersedes 3.7 as the quota fallback.
  it("defaults the Flash fallback to the GA gemini-3.8-flash model", () => {
    expect(MODELS.FLASH).toBe("gemini-3.8-flash");
  });

  it("references the current Flash model (not a superseded one) in the quota-exceeded message", () => {
    expect(ERROR_MESSAGES.QUOTA_EXCEEDED_SHORT).toContain("gemini-3.8-flash");
    expect(ERROR_MESSAGES.QUOTA_EXCEEDED_SHORT).not.toContain("gemini-3.7-flash");
  ...
  // #312 — 3.8 Flash is a quota fallback, not a Pro replacement.
  it("does not change the Pro factory default when adopting the 3.8 Flash fallback", () => {
  ...
  it("marks usage.fellBack=true after a Flash fallback", async () => {
    mockExecuteCommand.mockRejectedValueOnce(new Error("RESOURCE_EXHAUSTED")).mockResolvedValueOnce(
      JSON.stringify({
        ...
        stats: { models: { "gemini-3.8-flash": { tokens: { input: 10, candidates: 5, cached: 0, thoughts: 0 } } } },
      }),
    );
