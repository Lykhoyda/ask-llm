...
export const ERROR_MESSAGES = {
  ...
  QUOTA_EXCEEDED_SHORT: "⚠️ Gemini Pro daily quota exceeded. Please retry with model: 'gemini-3.7-flash'",
  ...
  WORKSPACE_TRUST_REQUIRED:
    "Gemini blocked this call because workspace-trust enforcement is enabled and the current directory is not trusted. To resolve, either (a) unset ASK_GEMINI_REQUIRE_WORKSPACE_TRUST so @ask-llm/gemini-mcp re-applies its safe default of co-emitting GEMINI_CLI_TRUST_WORKSPACE=true (≥0.42) and GEMINI_TRUST_WORKSPACE=true (≤0.41), (b) export GEMINI_CLI_TRUST_WORKSPACE=true yourself on gemini ≥0.42 (or GEMINI_TRUST_WORKSPACE=true on ≤0.41), (c) pass --skip-trust to gemini for this session (≥0.41), or (d) run `gemini` interactively in this directory once and mark it trusted.",
  TIER_DISCONTINUED:
    "⚠️ Gemini CLI tier change (2026-06-18): Google stopped serving Gemini CLI requests for free, Google AI Pro, and Ultra accounts on 2026-06-18; only Gemini Code Assist Standard/Enterprise seats remain supported. If you are on a free/Pro/Ultra account, the error below is LIKELY caused by that change (it can also be a genuine auth, billing, or quota error). Options: (a) upgrade to Gemini Code Assist Standard/Enterprise; (b) switch providers — use ask-codex or ask-ollama; (c) Google's successor is the separate Antigravity CLI (`agy`), which @ask-llm/gemini-mcp does not wrap — install the dedicated @ask-llm/antigravity-mcp package (https://www.npmjs.com/package/@ask-llm/antigravity-mcp, subscription-backed via your Google AI Pro/Ultra plan) or run `agy` directly. The npm package still installs and launches, so reinstalling will not help. See https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/",
} as const;
...
export const MODELS = {
  ...
  FLASH: process.env.ASK_GEMINI_FALLBACK_MODEL || "gemini-3.7-flash",
};
