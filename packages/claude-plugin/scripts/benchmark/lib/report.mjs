// Source of truth: report.mts. The sibling .mjs is generated by `yarn workspace @ask-llm/plugin build:hooks`; never edit it.
export function renderReport({ fixtures, runs, meta, }) {
    ...
    for (const arm of meta.arms) {
        const r = runs[arm];
        const totalShouldFlag = fixtures.reduce((a, f) => a + (r[f.name]?.score?.caught.length ?? 0) + (r[f.name]?.score?.missed.length ?? 0), 0);
    ...
    if (anyArmAllErrored) {
        ...
        lines.push("");
        lines.push(`> Decision rule: ship the change if recall delta ≥ +10 pp AND extra-finding delta ≤ +1 per fixture on average. If recall regresses or noise jumps, reverse ADR-099 per its documented two-file rollback path.`);
        lines.push("");
    }
        ...
        for (const arm of meta.arms) {
            ...
            if (s.extra.length > 0) {
                ...
                for (const e of s.extra) {
                    ...
                    if (e.body)
                        lines.push(`  └─ ${e.body.slice(0, 200)}${e.body.length > 200 ? "…" : ""}`);
                }
                lines.push("");
            }
        }
