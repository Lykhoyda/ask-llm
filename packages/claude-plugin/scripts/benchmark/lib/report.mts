...
// Markdown A/B report: per-fixture comparison plus aggregate recall delta.

import type { Probes, Score } from "./score.mts";
...
export function renderReport({
  ...
  meta: { timestamp: string; model: string; arms: string[] };
}): string {
  const lines: string[] = [];
  ...
  const aggregate: Record<string, { recall: number; caught: number; missed: number; extra: number }> = {};
  ...
  if (anyArmAllErrored) {
    ...
    const [armA, armB] = meta.arms as [string, string];
