...
export function isBrokerEnabled(markerDir) {
    ...
    if (!existsSync(join(state.isolatedHome, "auth.json")))
        return false;
...
import { isIsolatedBrokerHome, isPidAlive as lifecycleIsPidAlive, readBrokerDescriptorSync as lifecycleReadBrokerDescriptor, } from "./broker-lifecycle.mjs";
