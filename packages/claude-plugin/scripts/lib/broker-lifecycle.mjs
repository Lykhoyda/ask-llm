...
// lives in `broker.mts` so the per-edit hook can also use it as a
...
import { chmodSync, existsSync, mkdirSync, mkdtempSync, openSync, readFileSync, realpathSync, renameSync, rmSync, statSync, symlinkSync, unlinkSync, writeFileSync, } from "node:fs";
...
export async function bootstrapBroker(markerDir, options = {}) {
    ...
    if (!existsSync(join(sourceCodexHome(options.sourceHome), "auth.json")))
        return null;
    ...
    try {
        ...
        if (previous) {
            ...
            if (!expired && (injectDeps?.isRecordedBroker ?? isRecordedBroker)(previous))
                return previous;
