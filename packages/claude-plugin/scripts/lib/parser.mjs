...
export function buildVerdictMessage({ filePath, concerns, fellBack, durationMs, surfaceThreshold, cached, repeatedIgnoredCount = 0, logPath, }) {
    ...
    const details = [];
    for (const c of concerns.high)
        details.push(`[HIGH]\n${c}`);
    if (threshold === "med" || threshold === "low") {
        for (const c of concerns.med)
            details.push(`[MED]\n${c}`);
    }
    if (threshold === "low") {
        for (const c of concerns.low)
            details.push(`[LOW]\n${c}`);
    }
...
export function parseConcernsLegacy(message) {
    ...
    for (const part of parts) {
        ...
        if (label === "high")
            concerns.high.push(body);
        else if (label === "med")
            concerns.med.push(body);
        else if (label === "low")
            concerns.low.push(body);
    }
    return concerns;
}
...
export function parseResetHint(text) {
    ...
    for (const re of RESET_HINT_PATTERNS) {
        ...
        if (m) {
            ...
            if (/^\d+$/.test(hint))
                continue;
            if (hint.length > 0 && hint.length <= RESET_HINT_MAX_CHARS)
                return hint;
        }
    }
    return null;
