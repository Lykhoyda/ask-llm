...
import { hashConcernBody } from "./state.mts";
...
export function collectInFlight({
  ...
  for (const r of records as DebounceRecordLike[]) {
    ...
    if (Number.isFinite(r.burstStartedAt) && now - (r.burstStartedAt as number) > staleMs) continue;
...
export function formatInFlightMessage(
  { settling, reviewing }: { settling: string[]; reviewing: number },
  markerDir: string,
): string {
  const parts: string[] = [];
