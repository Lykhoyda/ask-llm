// Source of truth: debounce-state.mts. The sibling .mjs is generated by `yarn workspace @ask-llm/plugin build:hooks`; never edit it.
// Per-file edit-debounce state under .codex-pair/state/ (design 2026-06-03); every write is best-effort (ADR-077).
...
export function decideReview({ record, myGeneration, now, maxMs, }) {
    if (!record)
        return { review: false, reason: "record-missing" };
    if (record.reviewedGen >= myGeneration)
        return { review: false, reason: "already-reviewed" };
    if (record.generation === myGeneration)
        return { review: true, reason: "settled" };
    if (now - record.burstStartedAt >= maxMs)
        return { review: true, reason: "max-cap" };
...
// Caps only the surfaced text; the overflow stays in the log.
export const MAX_SURFACE_VERDICTS = 8;
export function joinPendingForSurface(messages) {
    if (messages.length <= MAX_SURFACE_VERDICTS)
