#!/usr/bin/env node
// Source of truth: codex-pair-log.mts. The sibling .mjs is generated by `yarn workspace @ask-llm/plugin build:hooks`; never edit it.
...
import { CONTEXT_FILENAME, PAIR_ROOT_DIR, logPath as resolveLogPath } from "./lib/state.mts";
...
function parseArgs(argv: string[]): Args {
  const args: Args = { cmd: "latest", n: 10, file: null, sinceMs: null };
...
function loadEntries(logPath: string): LogEntry[] {
  let content: string;
  try {
...
function applySinceFilter(entries: LogEntry[], sinceMs: number | null): LogEntry[] {
...
function formatCounts(entry: LogEntry): string {
  if (entry.counts) {
...
function showLatest(entries: LogEntry[], n: number): void {
...
function showSummary(entries: LogEntry[]): void {
