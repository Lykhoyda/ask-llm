...
import { prepareCommandInvocation } from "./lib/process.mts";
...
type SpawnProcess = (command: string, args: string[], options: SpawnOptions) => ChildProcessWithoutNullStreams;
...
export const UNIFIED_CODEX_OPTION_KEYS: string[] = ["reasoningEffort", "includeDirs", "preferred", "sandbox"];
...
export function isAskCodexToolName(name: string): boolean {
  return name === ASK_CODEX_TOOL || /^mcp__.+__ask-codex$/.test(name);
}

export function isAskCodexRegistration(server: McpServer | null | undefined): boolean {
...
export function isAskLlmToolName(name: string): boolean {
  return name === ASK_LLM_TOOL || /^mcp__.+__ask-llm$/.test(name);
}

export function isAskLlmRegistration(server: McpServer | null | undefined): boolean {
  if (!server || typeof server !== "object") return false;
  const command = typeof server.command === "string" ? server.command : "";
  const args = Array.isArray(server.args) ? server.args.filter((arg) => typeof arg === "string") : [];
  const commandLine = typeof server.commandLine === "string" ? server.commandLine : "";
...
export function unifiedSchemaHonorsCodexOptions(schema: { properties?: unknown } | null | undefined): boolean {
  const properties = schema?.properties;
  if (!properties || typeof properties !== "object") return false;
  return UNIFIED_CODEX_OPTION_KEYS.every((key) => Object.hasOwn(properties as object, key));
}

export function parseClaudeMcpList(output: string): Record<string, { commandLine: string; status: string }> {
  const servers: Record<string, { commandLine: string; status: string }> = {};
...
export function readActiveMcpServers({
  ...
  execute = spawnSync as SyncExecute,
  ...
  platform?: string;
  contextArgs?: string[];
} = {}): Record<string, { commandLine: string; status: string }> {
...
function expectedToolName(serverName: string, tool: string = ASK_CODEX_TOOL): string {
  return `mcp__${serverName.replaceAll(":", "_")}__${tool}`;
}

function isAvailableMcpServer(server: McpServer | undefined): boolean {
...
export function classifySolReviewTransport({
  ...
  mcpFailed?: boolean;
  toolSchemas?: Record<string, { properties?: unknown } | undefined>;
}): TransportDecision {
  if (inventoryError) {
  ...
  const reason = mcpFailed
    ...
    : unavailableRegistration
      ? `Ask LLM Codex MCP is registered, but active inventory reports it unavailable: ${unavailableRegistration[1].status as string}.`
      : registered
...
export function codexFallbackArgs(model: string): string[] {
  return [
    "exec",
    "-m",
    model,
...
export async function runCliFallback({
  ...
  execute?: (options: { command: string; model: string; prompt: string }) => Promise<CodexRun>;
}): Promise<{ response: string; diagnostics: string; model: string; fellBack: boolean }> {
...
interface ParsedArgs {
  ...
  fallback: boolean;
  mcpFailed: boolean;
  toolSchema: { properties?: unknown } | null;
  claudeContextArgs: string[];
}
...
function readMcpServers(parsed: ParsedArgs): {
  mcpServers: Record<string, McpServer>;
  inventoryError: string | null;
} {
...
async function main(): Promise<void> {
  ...
  const toolSchemas: Record<string, { properties?: unknown }> = {};
  if (parsed.toolSchema) {
    for (const name of parsed.tools) toolSchemas[name] = parsed.toolSchema as { properties?: unknown };
  }
