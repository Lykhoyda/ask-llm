#!/usr/bin/env node
// Source of truth: sol-review-transport.mts. The sibling .mjs is generated by `yarn workspace @ask-llm/plugin build:hooks`; never edit it.
...
const MISSING_REGISTRATION_REMEDIATION = "Run `claude mcp add --scope user ask-llm -- npx -y @ask-llm/mcp`, fully restart Claude Code, then verify with `/mcp`. Split Codex remains an advanced optimization: `claude mcp add --scope user codex -- npx -y @ask-llm/codex-mcp`.";
const SERVICE_REMEDIATION = "Run `npx -y @ask-llm/mcp doctor`, inspect `/mcp`, then fully restart Claude Code.";
const UPGRADE_UNIFIED_REMEDIATION = "Upgrade `@ask-llm/mcp` with `npx -y @ask-llm/mcp@latest` or `npm install -g @ask-llm/mcp`, fully restart Claude Code, then verify with `/mcp`.";
...
export function isAskCodexRegistration(server) {
    ...
    return (/(?:^|[/\\])ask-codex-mcp(?:\.cmd|\.exe)?$/.test(command) ||
...
export function isAskLlmRegistration(server) {
    ...
    const args = Array.isArray(server.args) ? server.args.filter((arg) => typeof arg === "string") : [];
    const commandLine = typeof server.commandLine === "string" ? server.commandLine : "";
    return (/(?:^|[/\\])ask-llm-mcp(?:\.cmd|\.exe)?$/.test(command) ||
...
export function readActiveMcpServers({ command = process.env.CLAUDE_BIN || "claude", execute = spawnSync, platform = process.platform, contextArgs = [], } = {}) {
    const invocation = prepareCommandInvocation([...contextArgs, "mcp", "list"], {
        cwd: process.cwd(),
        encoding: "utf8",
        windowsHide: true,
    }, platform);
...
export function classifySolReviewTransport({ availableTools = [], mcpServers = {}, cliPath = "", inventoryError = null, mcpFailed = false, toolSchemas = {}, }) {
    if (inventoryError) {
    ...
    const registeredCodexToolNames = new Set(availableCodexRegistrations.map(([name]) => expectedToolName(name, ASK_CODEX_TOOL)));
    const askCodexToolName = availableTools.find((name) => isAskCodexToolName(name) && registeredCodexToolNames.has(name));
    ...
    const registeredUnifiedToolNames = new Set(availableUnifiedRegistrations.map(([name]) => expectedToolName(name, ASK_LLM_TOOL)));
    ...
    const unavailableRegistration = codexRegistrations.find(([, server]) => !isAvailableMcpServer(server)) ||
...
export function executeCodex({ command, model, prompt, spawnProcess = spawn, platform = process.platform, }) {
    return new Promise((resolveRun) => {
        const invocation = prepareCommandInvocation(codexFallbackArgs(model), {
            cwd: process.cwd(),
            env: process.env,
...
export async function runCliFallback({ prompt, command = process.env.ASK_CODEX_BIN || "codex", fallbackModel = process.env.ASK_CODEX_FALLBACK_MODEL || TERRA_MODEL, execute = executeCodex, }) {
...
function parseArgs(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        else if (arg === "--mcp-failed")
            parsed.mcpFailed = true;
        else if (arg === "--tool")
            parsed.tools.push(args[++index] ?? "");
        else if (arg === "--cli-path")
            parsed.cliPath = args[++index] ?? "";
        else if (arg === "--mcp-list")
            parsed.mcpList = args[++index] ?? "";
        ...
        else if (arg === "--strict-mcp-config")
            parsed.claudeContextArgs.push(arg);
...
async function main() {
    ...
    if (parsed.toolSchema) {
        for (const name of parsed.tools)
            toolSchemas[name] = parsed.toolSchema;
    }
    ...
    if (parsed.fallback) {
        ...
        for await (const chunk of process.stdin)
            prompt += chunk;
        if (!prompt.trim())
            throw new Error("Sol review CLI fallback requires a prompt on stdin.");
        const result = await runCliFallback({ prompt, command: parsed.cliPath });
        process.stderr.write(`Transport disclosure: review ran through codex exec (${result.model}, high effort, read-only).\n`);
        if (result.fellBack) {
