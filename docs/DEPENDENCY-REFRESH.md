...
- Changesets CLI 3 removes Yarn Classic support, raises its own Node requirement to `^22.11 || ^24 || >=26`, and renames `changeset tag` to `changeset git-tag`. This repository uses Yarn 4, does not call `changeset tag`, and requires Node 24+ for a full build; the provider and orchestrator packages retain Node 20 runtime support. Publish now shells out to `yarn npm publish`, which does not read `NODE_AUTH_TOKEN` / setup-node's `.npmrc`; `release.yml` must pass `YARN_NPM_AUTH_TOKEN` (ADR-156 / issue #307). Sources: [CLI 3.0.0 release](https://github.com/changesets/changesets/releases/tag/%40changesets%2Fcli%403.0.0) and [GitHub changelog 1.0.0 release](https://github.com/changesets/changesets/releases/tag/%40changesets%2Fchangelog-github%401.0.0).
...
- **Provider and orchestrator Node support remains `>=20.0.0`; the plugin requires `>=24.0.0`.** The full repository build needs Node 24 because it includes the plugin. The packed-package matrix still boots the provider and orchestrator CLIs on Node 20 and Node 26; the plugin's Node floor is recorded in ADR-170.
