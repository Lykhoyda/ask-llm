...
- **Codex Astra:** `gpt-6-astra` requires Codex CLI >= 0.153.0 (`ASTRA_MIN_CODEX_VERSION` in `packages/codex-mcp/src/constants.ts`). The executor probes `codex --version` only for that slug; Sol/Terra stay ungated. `codex-pair` uses `CODEX_PAIR_MODEL` (`gpt-6-sol`). Ground truth: ADR-163, ADR-164.
