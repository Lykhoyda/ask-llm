...
| 6 | Structured errors and exit codes | **Pass (pilot); root dispatcher gap closed by #280** | `doctor --format toon --wat` produces `error.code: unknown_argument`, hint, empty stdout, and exit 2. `machine` already uses 0/2/3. At pilot time `ask-llm-mcp --help` and an unknown top-level command started provider detection/MCP instead of rejecting the command; issue #280 later added the root dispatcher contract (unsupported arguments print usage and exit 2 without starting the server — pinned in `packages/llm-mcp/src/__tests__/cli.test.ts`). No mutations are in this pilot. |
...
| 10 | Consistent help | **Pass (pilot); repository gap closed by #280** | `doctor --help` documents formats and `--full`; REPL has `/help`. Probe evidence at pilot time: `ask-llm-mcp --help` and `ask-llm-mcp unknown-command` both timed out after beginning `agy --version`, with no help. Issue #280 later added top-level `--help`/`--version` that print and exit without provider loading. |
