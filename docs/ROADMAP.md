...
### 2026-06-04 — codex-pair edit-debounce (branch `feat/codex-pair-edit-debounce`, closes #96)

Shipped the last open item of #96 (Bug 2 / Idea 1): a per-file edit **debounce** that collapses a burst of rapid same-file edits into a single review of the settled state. Architecture (ADR-112): a detached delayed-worker carrying a `generation` token — on each edit the hook records + spawns a sleeper; only the latest-generation worker (or one past the 60s cap) survives to review, by **re-invoking the hook in `CODEX_PAIR_FORCE_SYNC=1` mode** and queuing the verdict for the next PostToolUse drain or a new `UserPromptSubmit` drain. ON by default (15s settle / 60s cap; `debounceMs:0` restores synchronous v0.7.0 behavior). Built TDD across 8 tasks (39 new tests; 354 plugin tests green, lint/types clean) with a live real-codex smoke confirming 3 edits → 1 review, trailing-verdict surfacing, and the synchronous regression path. New files: `scripts/lib/debounce-state.mjs`, `scripts/codex-pair-debounce-worker.mjs`, `scripts/codex-pair-prompt-drain.mjs` (+ `UserPromptSubmit` hook). Specs: `docs/plans/2026-06-03-codex-pair-edit-debounce-{design,plan}.md`. Not yet merged.
