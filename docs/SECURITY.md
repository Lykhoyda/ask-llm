...
```bash
npm install -g @ask-llm/gemini-mcp@latest @ask-llm/codex-mcp@latest @ask-llm/claude-mcp@latest @ask-llm/grok-mcp@latest @ask-llm/ollama-mcp@latest @ask-llm/antigravity-mcp@latest @ask-llm/mcp@latest
```
...
- Affected package and version (`@ask-llm/gemini-mcp`, `@ask-llm/codex-mcp`, `@ask-llm/claude-mcp`, `@ask-llm/grok-mcp`, `@ask-llm/ollama-mcp`, `@ask-llm/antigravity-mcp`, `@ask-llm/mcp`, or `@ask-llm/plugin`)
...
The MCP server runs locally as a subprocess of the user's MCP client (Claude Code, Claude Desktop, Cursor, etc.) with the user's privileges. The `@ask-llm/plugin` Pi extension likewise runs with the installing user's full permissions; Pi project trust is an input-loading guard, not a sandbox. Provider CLIs (`gemini`, `codex`, `claude`, `agy`), Ollama, and the fixed `https://api.x.ai` Grok API origin are trusted dependencies. Grok credentials are read only from `XAI_API_KEY`, sent only in the Authorization header to that fixed origin, redacted from bounded diagnostics, and never returned in response metadata. Grok API requests disable server-side response storage and do not enable tools or priority processing. The Grok CLI harness runs one read-only sandboxed turn. The model-neutral Cursor Agent harness requires separate provider/model attribution, uses ask mode, and never auto-passes `--force` or `--trust`; `CURSOR_API_KEY` is redacted like the xAI key.
...
- Vulnerabilities in upstream provider CLIs/APIs — report to Google (Gemini/Antigravity), OpenAI (Codex), Anthropic (Claude), xAI (Grok), Cursor (Cursor Agent), or Ollama directly
