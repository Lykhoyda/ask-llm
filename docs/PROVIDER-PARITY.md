...
| Structured output | prompt/changeMode paths | native `--output-schema` | prompt-constrained | xAI Responses `text.format` strict JSON Schema; CLI is prompt-constrained with the same schema; both transports are validated once locally by the shared machine boundary (`parseRolePayload` + refinements); API structured calls bypass cache | prompt-constrained | prompt-constrained |
