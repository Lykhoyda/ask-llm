...
| includeDirs | `ask-gemini-edit` only (`--include-directories`), validated via shared `relativeDirSchema` | `ask-codex` + `ask-codex-edit` (`--add-dir`), validated (2026-07-02) | n/a (HTTP; no file access) | `--add-dir` supported by agy; not exposed as a tool param |
