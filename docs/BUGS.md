...
### ~~codex-pair reports "injected review prompt" HIGHs on files containing `` $` ``~~ FIXED (ADR-165, #281)
...
- **Repro:** edit any file whose text contains `` $` `` (for example a markdown bullet listing glyphs as `` `$` ``). The per-edit review reports a HIGH claiming "the entire review prompt / `<file_content>` markers were pasted into the file" at that line. Observed on `docs/superpowers/specs/2026-07-13-docs-overhaul-design.md:67` and `docs/superpowers/plans/2026-07-14-docs-overhaul-terminal-noir.md:23`; both lines contain `` $` ``.
- **Root cause:** not the reviewer, and not the box/arrow glyphs (the original hypothesis). `buildReviewPrompt` substituted template tokens with chained `String.prototype.replace(token, value)` calls, so `$` replacement patterns in the file or context were interpreted: `` $` `` inserted the whole prompt prefix into the `<file_content>` payload, and `$&`, `$'` and `$$` altered the text. The reviewer was correctly reporting the corrupted payload it received. A `{{FILE_CONTENT}}` token inside `context.md` also captured the file insertion. Glyph-only content always rendered unchanged.
- **Status:** **FIXED** (ADR-165): the production renderer and the benchmark renderer substitute all tokens in one pass with a function replacer, so values are inserted literally and never re-scanned. `src/__tests__/review-prompt.test.ts` pins each `$` pattern, both incident lines, a token inside context, and glyph-only content; the ADR-089 golden fixture is unchanged. The separate observation that stale `context.md` produced two false "X doesn't exist" MEDs is not addressed here.
