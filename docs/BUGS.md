...
- **Root cause and fix:** The renderer interpreted `$` patterns in replacement values; the box/arrow glyphs were unrelated. See ADR-165 for the rendering contract and `src/__tests__/review-prompt.test.ts` for regressions.
- **Status:** **FIXED**. The separate observation that stale `context.md` produced two false "X doesn't exist" MEDs is not addressed here.
