...
- **Related (NOT fixed here):** the published `ask-codex-mcp` executor (`codexExecutor.ts:419`) shares the same `gpt-5.5 → gpt-5.5-mini` fallback assumption, but already surfaces a clear terminal error ("quota exceeded, fallback also failed: …") rather than cascading — acceptable, left as-is.
